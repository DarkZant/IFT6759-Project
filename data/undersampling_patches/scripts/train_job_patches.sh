...
echo "Strategy   : Patch-based 128x128, BG ratio 30%"
echo "========================================"
...
# ── Copie des données vers SSD local ──────────────────────────────────────────
...
# Pour le test on garde TOUS les patches (évaluation complète)
python $REPO_DIR/preprocess_patches.py \
...
echo "patches 128x128 BG=30% | $(date) | job=$SLURM_JOB_ID" > $OUTPUT_DIR/run_info.txt
...
python $REPO_DIR/train_patches.py \

...
from climatenet.utils.losses import jaccard_loss, weighted_jaccard_loss
...
parser.add_argument('-d', '--data', required=True, help='Dossier patches (contient train/, val/)')
parser.add_argument('-n', '--nc_dir', required=True, help='Dossier .nc originaux (contient val/, test/)')
parser.add_argument('-o', '--output', required=True)
parser.add_argument('--tc_oversample', type=int, default=1, help='Facteur oversampling TC')
parser.add_argument('--ce_weights', type=float, nargs=3, default=None, metavar=('BG', 'TC', 'AR'))
parser.add_argument('--wjaccard_weights', type=float, nargs=3, default=None, metavar=('BG', 'TC', 'AR'))
args = parser.parse_args()

config  = Config(args.config)
network = CGNetModule(classes=3, channels=16).cuda()
...
if args.ce_weights:
    ...
    loss_mode = 'ce'
elif args.wjaccard_weights:
    loss_mode = 'wjaccard'
else:
    loss_mode = 'jaccard'

train_set = PatchDataset(path.join(args.data, 'train'), config, tc_oversample=args.tc_oversample)
val_set = PatchDataset(path.join(args.data, 'val'), config, tc_oversample=1)
    ...
    for f in tqdm(files, desc=name):
        ...
        for r0 in row_starts:
            for c0 in col_starts:
                ...
                X_patch = torch.FloatTensor(X_full[:, r0:r1, c0:c1]).unsqueeze(0).cuda()
        ...
        aggregate_cm += get_cm(torch.tensor(pred_full), torch.tensor(y_full), 3)
...
for epoch in range(config.epochs):
    ...
    for X, y in epoch_loader:
        ...
        elif loss_mode == 'wjaccard':
            outputs = torch.softmax(network(X), 1)
            loss = weighted_jaccard_loss(outputs, y, weights=args.wjaccard_weights)
        else:
            outputs = torch.softmax(network(X), 1)

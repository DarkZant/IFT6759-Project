...
class PatchDataset(Dataset):
    ...
    augment : bool
        Si True, applique des augmentations aléatoires sur TOUS les patches
        pendant l'entraînement (rotation + flip)
    """

    VARIABLES = ['TMQ', 'U850', 'V850', 'PSL']

    # Augmentations disponibles : (k rotations 90°, flip horizontal)
    AUGMENTATIONS = [
        (0, False),   # original
        (1, False),   # rotation 90°
    ...
    def __init__(self, patch_dir: str, config,
        ...
        # Oversampling TC avec augmentation assignée
        # Chaque copie reçoit un index d'augmentation différent
        tc_oversampled = []
        ...
        print(f'  Après oversampling (TC×{tc_oversample}) :')
        print(f'    TC={n_tc_eff:,}  AR={len(ar_files):,}  BG={len(bg_files):,}  Total={len(self.entries):,}')
        print(f'  Augmentation aléatoire : {augment}')
        print(f'  Normalisation : {self.VARIABLES}')

    def __len__(self):
        return len(self.entries)

    def _apply_augmentation(self, X: torch.Tensor, y: torch.Tensor,
                             aug_idx: int):
        """
        Applique une augmentation spécifique.
        X : (4, H, W), y : (H, W)
        aug_idx : index dans AUGMENTATIONS
        """
    ...
    def __getitem__(self, idx: int):
        ...
        X = torch.from_numpy(patch['X']).float()   # (4, H, W)
        ...
        if aug_idx is not None:
            ...
            aug_idx_rand = random.randint(0, len(self.AUGMENTATIONS) - 1)
            X, y = self._apply_augmentation(X, y, aug_idx_rand)
